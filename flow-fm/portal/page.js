...
const wombWorkPanel=document.getElementById('womb-work');
const busyWorkPanel=document.getElementById('busy-work');
...
function busyWorkLinks(record){
  const submission=safeHref(record?.submission_url);
  const attachment=safeHref(record?.attachment_url);
  const links=[];
  if(submission) links.push(`<a href="${escapeHtml(submission)}" target="_blank" rel="noreferrer">Open Busy Work link</a>`);
...
function renderBusyWorkForm(portal, record, readOnly){
  const busyWork=portal.busyWork || portal.businessAssignment;
  if(!busyWork) return '<p>This Moon is reserved for integration.</p>';
  if(Number(busyWork.index)===1){
    return `<div class="assignment-complete-note"><p>The Priestess Profile Studio is the workroom for Busy Work 1. Submit the profile there; the Busy Work tracker should treat that as the real doorway.</p><div class="assignment-actions"><a class="pill-link" href="/flow-fm/profile-studio/">Open Profile Studio</a></div>${witnessNotes(record)}</div>`;
  }
  if(readOnly){
    return `<div class="assignment-readonly">${record?.submission_text ? `<p>${escapeHtml(record.submission_text)}</p>` : `<p class="empty-note">This Busy Work is open to explore. Sign in as the member to save or submit.</p>`}${busyWorkLinks(record)}${witnessNotes(record)}</div>`;
  }
  if(record?.status === 'complete'){
    return `<div class="assignment-complete-note"><p>${escapeHtml(assignmentStatusCopy(record.status))}</p>${record?.submission_text ? `<p>${escapeHtml(record.submission_text)}</p>` : ''}${busyWorkLinks(record)}${witnessNotes(record)}</div>`;
  }
  return `<form class="assignment-form" data-assignment-form="${escapeHtml(busyWork.index)}">
    ...
    <div class="form-grid"><label><span>Busy Work link</span><input name="submission_url" type="url" placeholder="https://..." value="${escapeHtml(record?.submission_url || '')}" /></label><label><span>File or media URL</span><input name="attachment_url" type="url" placeholder="https://..." value="${escapeHtml(record?.attachment_url || '')}" /></label></div>
    ${witnessNotes(record)}
...
function bindBusyWorkForm(){
  const form=busyWorkPanel.querySelector('[data-assignment-form]');
  if(!form) return;
  form.querySelectorAll('[data-assignment-action]').forEach(button=>button.addEventListener('click',()=>handleBusyWorkAction(form,button.dataset.assignmentAction)));
}
async function handleBusyWorkAction(form,action){
  ...
  try{
    setMessage(message, action==='submit' ? 'Sending this Busy Work to be witnessed...' : 'Saving this Busy Work draft...');
    ...
    setMessage(message, action==='submit' ? 'Busy Work sent to be witnessed.' : 'Busy Work draft saved.');
  }catch(error){
    console.error(error);
    setMessage(message, error.message || 'This Busy Work could not be tended yet.');
  }
}
function renderPortal(portal){
  ...
  const busyWork=portal.busyWork || portal.businessAssignment || {};
  const record=recordForIndex(currentRecords, busyWork.index);
  ...
  portalIntro.textContent=portal.isOuroboros
    ...
    : `This portal gathers ${portal.name}, ${module.title || 'womb work'}, and ${busyWork.title || 'Busy Work'} in one place.`;
  hereTitle.textContent=portal.name;
  hereCopy.textContent=portal.isCurrent
    ? 'This is the moon Flowtel is currently orienting you through. Training, practice, and the Busy Work doorway are gathered below.'
  ...
  wombWorkPanel.innerHTML=`<p class="eyebrow">WOMB WORK MODULE ${escapeHtml(module.index || portal.portalIndex)}</p><h3>${escapeHtml(module.title || 'Integration')}</h3><p>${escapeHtml(module.description || 'Integration practices live here.')}</p><div class="module-detail-grid"><article><span>Practice</span><p>${escapeHtml(module.practice || 'Practice will be added here.')}</p></article><article><span>Reflection Prompt</span><p>${escapeHtml(module.prompt || 'Prompt will be added here.')}</p></article><article><span>Course Content</span><p><span class="lesson-placeholder">Squarespace lesson placeholder</span></p></article></div>${Number(module.index || portal.portalIndex)===1 ? '<div class="module-cta-row"><a class="pill-link" href="/tracker/">Track Your Cycle</a></div>' : ''}`;
  busyWorkPanel.innerHTML=`<p class="eyebrow">BUSY WORK ${escapeHtml(busyWork.index || portal.portalIndex)}</p><div class="assignment-row-heading"><div><h3>${escapeHtml(busyWork.title || 'Integration')}</h3><p>${escapeHtml(busyWork.description || 'This practical build completes the spiral.')}</p></div>${statusPill(record?.status || 'not_started')}</div><p class="assignment-status-copy">${escapeHtml(assignmentStatusCopy(record?.status || 'not_started'))}</p>${renderBusyWorkForm(portal, record, readOnly)}`;
  bindBusyWorkForm();
}
async function loadRecords(){
  if(!currentProfile){ currentRecords=[]; return; }
  try{ currentRecords=await listFlowFmAssignmentStatuses(requestedMemberId()); }
  catch(error){ console.error(error); currentRecords=[]; setMessage(message,'The portal opened, but Busy Work records could not be loaded.'); }
}
async function init(){
