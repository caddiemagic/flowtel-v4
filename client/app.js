...
let complimentaryStayAutoUpgradeChecked=false;
let complimentaryStayUpgradeNotice="";
let loungeWorkshopLoadPromise=null;
let loungeWorkshopLoadedVideoId=null;
let loungeSeasonPlan=null;
let loungeSeasonPlannerBusy=false;
let loungeEventsLoadPromise=null;
let loungeEvents=[];
...
function hasUnconvertedComplimentaryStay(){
  return Boolean(currentProductAccess?.flowtel_trial_started_at && !currentProductAccess?.flowtel_trial_converted_at);
}
...
function renderComplimentaryStayBanner(){
  ...
  if(copy) copy.textContent=day>=14
    ? "Your final complimentary day. Join the Queendom to keep your room open tomorrow."
    : day>=11
      ? "Your stay is almost complete. Join the Queendom to keep your room open."
      : "Your room is complimentary for fourteen days.";
}
...
async function convertComplimentaryStayMembership({statusElement=null,silent=false}={}){
  ...
  if(!silent&&status) status.textContent="Checking your Queendom room key…";
  ...
  if(hasUnconvertedComplimentaryStay() || currentProfileMembershipRank(currentProfile)<1){
  ...
  complimentaryStayUpgradeNotice="Welcome to the Queendom. Your room key has been upgraded.";
  if(status) status.textContent="Your Queendom room key is ready. Welcome home.";
  return true;
}

function isExpectedComplimentaryStayMembershipMiss(error){
  const message=String(error?.message||"").toLowerCase();
  return message.includes("no squarespace contact")
    || message.includes("could not verify an active queendom or flow fm membership purchase")
    || message.includes("use the email you used at checkout")
    || message.includes("purchase verification is not configured yet");
}

async function maybeAutoUpgradeComplimentaryStay(){
  if(complimentaryStayAutoUpgradeChecked||!hasUnconvertedComplimentaryStay()) return false;
  complimentaryStayAutoUpgradeChecked=true;
  try{
    return await convertComplimentaryStayMembership({silent:true});
  }catch(error){
    if(!isExpectedComplimentaryStayMembershipMiss(error)){
      console.warn("Complimentary stay automatic membership check could not complete.",error);
    }
    return false;
  }
}
...
async function handleTrialMembershipRefresh(statusElement=null){
  ...
  try{
    ...
    await convertComplimentaryStayMembership({statusElement:status});
...
function updatePhaseOneSuiteLinks(){
  ...
  if(wombMagicCard){
    wombMagicCard.classList.remove("hidden");
    wombMagicCard.classList.toggle("is-trial-locked",trial);
  }
  ...
  const wombMagicEyebrow=document.getElementById("wombMagicSuiteEyebrow");
  const wombMagicTitle=document.getElementById("wombMagicSuiteTitle");
  const wombMagicCopy=document.getElementById("wombMagicSuiteCopy");
  const wombMagicScheduleButton=document.getElementById("wombMagicScheduleButton");
  const wombMagicTrialCta=document.getElementById("wombMagicTrialCta");
  const wombMagicSummary=wombMagicCard?.querySelector("[data-wm-summary]");
  const wombMagicPanel=wombMagicCard?.querySelector("[data-wm-panel]");

  if(wombMagicEyebrow) wombMagicEyebrow.textContent=trial?"QUEENDOM MEMBER BENEFIT":"COMPLIMENTARY MENTOR CALL";
  if(wombMagicTitle) wombMagicTitle.textContent="Book your Womb Magic call.";
  if(wombMagicCopy) wombMagicCopy.textContent=trial
    ? "Queendom members can choose any available Flow FM Priestess for one private 45-minute Womb Magic call each month."
    : "Choose any available Flow FM Priestess for one private 45-minute call each month.";
  if(wombMagicScheduleButton) wombMagicScheduleButton.classList.toggle("hidden",trial);
  if(wombMagicTrialCta) wombMagicTrialCta.classList.toggle("hidden",!trial);
  if(trial){
    wombMagicBooking.close();
    if(wombMagicSummary) wombMagicSummary.hidden=true;
    if(wombMagicPanel){wombMagicPanel.hidden=true;wombMagicPanel.setAttribute("aria-hidden","true");}
  }
  ...
  document.querySelectorAll('a[href="https://www.theidyllcollective.com/queendomhome"]').forEach(link=>{
    if(link.closest(".complimentary-stay-expired-card")||link.closest(".complimentary-stay-lounge-card")||link.id==="wombMagicTrialCta"||link.classList.contains("join-queendom-doorway-button")) return;
...
function updateDoorwayCopy(){
  ...
  if(nonMemberDoorways) nonMemberDoorways.classList.toggle("hidden",Boolean(SQUARESPACE_MEMBERSHIP));
...
function memberBridgeEmail(){
  const newEmail=document.getElementById("newAccountEmail")?.value;
...
async function verifySquarespaceMember(email,intent="verify"){
  ...
  if(!response.ok || !data.ok) throw new Error(data.error || "Flowtel could not verify this Queendom email yet.");
  return data;
}
...
function showNewAccountForm(mode="member"){
  ...
  if(create){create.classList.remove("hidden");create.setAttribute("aria-hidden","false");}
  const email=document.getElementById("newAccountEmail");if(email&&!email.value)email.value=loginEmail||"";
  setMessage("");setTimeout(()=>email?.focus(),60);
}

function showLoginForm(){
  const login=document.getElementById("flowtelLoginCard"),create=document.getElementById("newAccountCard");
  const newEmail=document.getElementById("newAccountEmail")?.value;
  if(create){create.classList.add("hidden");create.setAttribute("aria-hidden","true");}
  if(login) login.classList.remove("hidden");
  const email=document.getElementById("email");if(email&&newEmail&&!email.value)email.value=newEmail;
  setMessage("");setTimeout(()=>email?.focus(),60);
}
...
async function registerPendingEventDoorway(){
  ...
  try{
    ...
    eventDoorwayMessage=/inside Flow FM/i.test(detail)
      ? "This gathering is inside Flow FM. You can still see what is happening, but registration is reserved for Flow FM members."
      : (detail || "Your seat could not be saved just now.");
  }
}

function cleanEventDoorwayUrl(){
...
function focusMyUpcomingEvents(){
  const wantsMyEvents=Boolean(eventDoorwayEventId)||Boolean(eventRoomEventId)||window.location.hash==="#my-upcoming-events";
  if(!wantsMyEvents) return;
  window.setTimeout(()=>{
    setLoungeEventsView("mine",{scroll:true});
    const status=document.getElementById("loungeEventsStatus");
    if(status && eventDoorwayMessage) status.textContent=eventDoorwayMessage;
    if(eventDoorwayEventId) cleanEventDoorwayUrl();
  },350);
}
...
async function continueAuthenticatedEntrance(){
  ...
  setAuthenticatedAccountVisible(true);
  await refreshCurrentProductAccess();
  await maybeAutoUpgradeComplimentaryStay();
  scheduleComplimentaryStayExpiry();
  ...
  setMessage(complimentaryStayUpgradeNotice);
  complimentaryStayUpgradeNotice="";
  ...
  pendingArrivalStay=null;
  showCheckIn();
  if(eventDoorwayEventId||eventRoomEventId){
    setMessage(eventDoorwayMessage
      ? `${eventDoorwayMessage} Check in for today and Flowtel will take you straight to My Upcoming Events.`
      : "Check in for today and Flowtel will take you straight to My Upcoming Events.");
  }
}
...
async function handleForgotPassword(){
  ...
  try{
    ...
    const redirectTo=`${window.location.origin}/client/?passwordRecovery=1`;
...
async function handleSwitchAccount(){
  ...
  window.location.replace(`/client/?forceDoorway=1`);
}
...
async function openRememberedRoomKey(){
  ...
  try{
    ...
    if(user.user_metadata?.flowtel_password_chosen && profileNeedsPersonalRoomKey(currentProfile)){
      currentProfile=await markPersonalRoomKeyCreated();
    }
...
async function handleSignIn(){
  try{
    ...
    currentProfile=await ensureProfile({
      squarespaceSource:"private-login",
    });
...
function maybeRouteToProfileConfirmation(){
  if(eventDoorwayEventId||eventRoomEventId) return false;
...
async function handleCheckIn(){
  try{
    ...
    setTimeout(()=>{
      ...
      setTimeout(()=>{
        renderSuite(stay);
        showScene((eventDoorwayEventId||eventRoomEventId)?"lounge":"suite");
        maybeRouteToProfileConfirmation();
      },950);
    },650);
  }catch(error){
...
document.getElementById("openMyUpcomingEventsButton")?.addEventListener("click",()=>{window.history.replaceState({},"",`${window.location.pathname}${window.location.search}#my-upcoming-events`);setLoungeEventsView("mine",{scroll:true});});
document.getElementById("backToUpcomingEventsButton")?.addEventListener("click",()=>{window.history.replaceState({},"",`${window.location.pathname}${window.location.search}`);setLoungeEventsView("discover",{scroll:true});});
document.getElementById("backToSuiteButton").addEventListener("click",()=>{showScene("suite");window.scrollTo({top:0,behavior:"smooth"});});
document.getElementById("eventRoomClose")?.addEventListener("click",closeEventRoom);
document.getElementById("eventRoomOverlay")?.addEventListener("click",event=>{if(event.target?.id==="eventRoomOverlay")closeEventRoom();});
document.addEventListener("keydown",event=>{if(event.key==="Escape"&&!document.getElementById("eventRoomOverlay")?.classList.contains("hidden"))closeEventRoom();});
...
const LOUNGE_SEASON_ORDER=['autumn','summer','winter','spring'];
...
function loungeProfileLink(memberId,name){const safe=loungeEscape(name||'Flow FM Priestess');return memberId?`<a href="/flow-fm/team-map/profile/?member=${encodeURIComponent(memberId)}">${safe}</a>`:safe;}
function loungeHostMarkup(event){
  const host=event?.host_name?loungeProfileLink(event.host_member_id,event.host_name):'';
  const co=event?.co_host_name?loungeProfileLink(event.co_host_member_id,event.co_host_name):'';
  if(!host&&!co)return'';
  const labels=[];if(host)labels.push(`Hosted by ${host}`);if(co)labels.push(`Co-hosted by ${co}`);
  const times=[];
  if(event?.host_timezone){const t=loungeTimestamp(new Date().toISOString(),event.host_timezone);if(t)times.push(`${loungeEscape(event.host_name)} · ${loungeEscape(t)}`);}
  if(event?.co_host_timezone){const t=loungeTimestamp(new Date().toISOString(),event.co_host_timezone);if(t)times.push(`${loungeEscape(event.co_host_name)} · ${loungeEscape(t)}`);}
  return `<div class="lounge-event-hosts"><span>${labels.join(' · ')}</span>${times.length?`<small>${times.join(' · ')}</small>`:''}</div>`;
}
...
function calendarEndParts(event){
  if(event.end_time)return {date:event.event_date,time:event.end_time};
  const [year,month,day]=String(event.event_date||'').split('-').map(Number);
  const match=/^(\d{2}):(\d{2})/.exec(String(event.start_time||''));
  if(!year||!month||!day||!match)return {date:event.event_date,time:event.start_time};
...
function eventRoomTimeMarkup(details){
  const flow=loungeTimestamp(details.starts_at,'America/Los_Angeles'),mine=currentProfile?.timezone&&currentProfile.timezone!=='America/Los_Angeles'?loungeTimestamp(details.starts_at,currentProfile.timezone):'';
  const liveAt=details.live_room_starts_at||details.starts_at,liveFlow=loungeTimestamp(liveAt,'America/Los_Angeles'),liveMine=mine?loungeTimestamp(liveAt,currentProfile.timezone):'';
  const distinct=String(liveAt)!==String(details.starts_at);
  return `<div class="event-room-times"><p><strong>EVENT START · FLOWTEL TIME</strong><br>${loungeEscape(flow)}</p>${mine?`<p><strong>EVENT START · YOUR TIME</strong><br>${loungeEscape(mine)}</p>`:''}${distinct?`<p><strong>LIVE GATHERING · FLOWTEL TIME</strong><br>${loungeEscape(liveFlow)}</p>${liveMine?`<p><strong>LIVE GATHERING · YOUR TIME</strong><br>${loungeEscape(liveMine)}</p>`:''}`:''}</div>`;
}
function eventRoomHostMarkup(details){
  const host=details.host_name?`<p><strong>Host</strong><br>${loungeProfileLink(details.host_member_id,details.host_name)}</p>`:'';
...
async function openRegisteredEventRoom(eventId){
  ...
  try{
    const event=loungeEvents.find(item=>String(item.event_id)===String(eventId));
    if(event?.access?.mode==='ticket'){
...
function closeEventRoom(){const overlay=document.getElementById('eventRoomOverlay');if(!overlay)return;overlay.classList.add('hidden');overlay.setAttribute('aria-hidden','true');document.body.classList.remove('event-room-is-open');}
...
function formatCountdown(ms){const total=Math.max(0,Math.floor(ms/1000)),m=Math.floor(total/60),s=total%60;return `${m}:${String(s).padStart(2,'0')}`;}
function loungeJoinEventLabel(event){return({workshop:'JOIN WORKSHOP',ceremony:'JOIN CEREMONY',call:'JOIN CALL',other:'OPEN EVENT'})[String(event?.event_type||'').toLowerCase()]||'OPEN EVENT';}
function renderLoungeLiveAlert(){
  const box=document.getElementById('loungeLiveEventAlert');if(!box)return;const candidate=liveEventCandidate();window.clearInterval(loungeEventCountdownTimer);loungeEventCountdownTimer=null;
  if(!candidate){box.classList.add('hidden');box.innerHTML='';return;}
  const draw=()=>{
    const left=candidate.at-Date.now(),isLive=candidate.phase==='live'||candidate.phase==='event-live';
    const action=isLive?loungeJoinEventLabel(candidate.event):'OPEN EVENT';
    const eyebrow=isLive&&candidate.phase==='live'?'LIVE GATHERING SOON':'STARTING SOON';
    const phrase=candidate.phase==='live'?' live gathering starts in ':' starts in ';
    const nowPhrase=candidate.phase==='live'?' live gathering is starting now.':' is starting now.';
    if(left<0){box.innerHTML=`<div><p class="eyebrow">${isLive?'YOUR ROOM IS OPEN':'YOUR EVENT IS STARTING'}</p><strong>${loungeEscape(candidate.event.title)}</strong><span>${nowPhrase}</span></div><button type="button" data-lounge-open-event="${loungeEscape(candidate.event.event_id)}">${loungeEscape(action)}</button>`;bindLoungeEventActions(box);return;}
    box.innerHTML=`<div><p class="eyebrow">${eyebrow}</p><strong>${loungeEscape(candidate.event.title)}</strong><span>${phrase}<b>${formatCountdown(left)}</b></span></div><button type="button" data-lounge-open-event="${loungeEscape(candidate.event.event_id)}">${loungeEscape(action)}</button>`;bindLoungeEventActions(box);
  };
  box.classList.remove('hidden');draw();loungeEventCountdownTimer=window.setInterval(draw,1000);
}
function bindLoungeEventActions(root){
  ...
  root?.querySelectorAll('[data-lounge-calendar-ics]').forEach(button=>button.addEventListener('click',()=>{const event=loungeEvents.find(item=>item.event_id===button.dataset.loungeCalendarIcs);if(event)downloadCommunityCalendar(event);}));
}
...
async function verifyPendingLoungeTicket(){
  let eventId='';try{eventId=String(localStorage.getItem('flowtel:pendingEventTicket')||'').trim();}catch(_){return null;}
  if(!eventId)return null;const event=loungeEvents.find(item=>String(item.event_id)===eventId);if(!event||event?.access?.mode!=='ticket'||event?.access?.entitled){try{localStorage.removeItem('flowtel:pendingEventTicket');}catch(_){ }return null;}
  try{const result=await verifyQueendomEventTicket(eventId);if(result?.paid||result?.revoked){try{localStorage.removeItem('flowtel:pendingEventTicket');}catch(_){ }}return result;}catch(error){console.warn('Pending Flowtel event ticket could not be verified yet.',error);return null;}
}
async function prepareLoungeEvents({force=false}={}){
  if(loungeEventsLoadPromise&&!force)return loungeEventsLoadPromise;loungeEventsLoadPromise=(async()=>{const status=document.getElementById('loungeEventsStatus');if(status)status.textContent='Opening the Queendom calendar…';try{const [events,womb]=await Promise.all([listQueendomEvents({monthCount:6}),loadWombMagicScheduling().catch(()=>null)]);loungeEvents=events;loungeWombMagicState=womb;const pendingTicket=await verifyPendingLoungeTicket();if(pendingTicket?.paid){loungeEvents=await listQueendomEvents({monthCount:6});eventDoorwayMessage='Your ticket is confirmed. Your seat is saved and your event room is ready.';}renderLoungeEvents();if(eventRoomEventId)window.setTimeout(()=>openRegisteredEventRoom(eventRoomEventId),100);return events;}catch(error){console.error('Flowtel Lounge events could not open or render.',error);if(status)status.textContent='Upcoming events could not open just now.';return[];}finally{loungeEventsLoadPromise=null;}})();return loungeEventsLoadPromise;
}
