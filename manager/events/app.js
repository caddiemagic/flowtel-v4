...
} from '/shared/queendom-events.js?v=0.10.83';
...
function eventDateLabel(row){
  ...
  return `${day} · ${start}${end?`–${end}`:''} · ${row.event_timezone||'America/Los_Angeles'}`;
}
...
function resetForm(){
  form.reset();fields.id.value='';fields.imagePath.value='';fields.imageUrl.value='';fields.timezone.value='America/Los_Angeles';fields.type.value='ceremony';fields.audience.value='queendom';fields.status.value='draft';fields.status.disabled=false;saveButton.disabled=false;cancelButton.hidden=true;message.textContent='';document.getElementById('eventEditorTitle').textContent='Create an event';setPreview('');
  const tomorrow=new Date(Date.now()+86400000);fields.date.value=tomorrow.toISOString().slice(0,10);
}
function editEvent(row){
  const cancelled=row.status==='cancelled';fields.id.value=row.event_id||'';fields.title.value=row.title||'';fields.type.value=row.event_type||'workshop';fields.audience.value=row.audience||'queendom';fields.date.value=row.event_date||'';fields.start.value=String(row.start_time||'').slice(0,5);fields.end.value=String(row.end_time||'').slice(0,5);fields.timezone.value=row.event_timezone||'America/Los_Angeles';fields.host.value=row.host_name||'';fields.description.value=row.description||'';fields.zoom.value=row.zoom_url||'';fields.passcode.value=row.zoom_passcode||'';fields.status.value=row.status==='cancelled'?'cancelled':(row.status==='published'?'published':'draft');fields.imagePath.value=row.image_path||'';fields.imageUrl.value=row.image_url||'';fields.imageFile.value='';cancelButton.hidden=row.status!=='published';saveButton.disabled=cancelled;fields.status.disabled=cancelled;message.textContent=cancelled?'Cancelled events stay in history and are read-only. Create a new event if this gathering returns.':'';document.getElementById('eventEditorTitle').textContent=cancelled?'Cancelled event':'Edit event';setPreview(row.image_url||'');window.scrollTo({top:0,behavior:'smooth'});
}
function rowMarkup(row){
  const image=row.image_url?`<img src="${esc(row.image_url)}" alt="">`:'<div class="event-admin-placeholder">✦</div>';
  const cancelled=row.status==='cancelled';
  return `<article class="event-admin-row ${cancelled?'is-cancelled':''}" data-event-id="${esc(row.event_id)}"><div class="event-admin-art">${image}</div><div class="event-admin-copy"><p class="eyebrow">${esc(typeLabel(row.event_type))} · ${esc(audienceLabel(row.audience))}</p><h3>${esc(row.title)}</h3><p>${esc(eventDateLabel(row))}</p><div class="event-admin-chips"><span>${esc(String(row.registration_count||0))} saved ${Number(row.registration_count||0)===1?'seat':'seats'}</span><span>${row.zoom_url?'Zoom placed':'Zoom waiting'}</span><span>${esc(String(row.status||'draft').toUpperCase())}</span></div></div><div class="event-admin-actions"><button type="button" data-edit-event>Edit</button>${row.status==='published'?'<button type="button" class="quiet-button" data-cancel-event>Cancel</button>':''}</div></article>`;
}
function render(){
...
function payload(){return{event_id:fields.id.value||null,title:fields.title.value,event_type:fields.type.value,description:fields.description.value,event_date:fields.date.value,start_time:fields.start.value,end_time:fields.end.value||null,timezone:fields.timezone.value,host_name:fields.host.value,audience:fields.audience.value,zoom_url:fields.zoom.value,zoom_passcode:fields.passcode.value,image_path:fields.imagePath.value,image_url:fields.imageUrl.value,status:fields.status.value};}
async function save(event){
  ...
  try{
    ...
    if(fields.imageFile.files?.[0]){
      message.textContent='Placing the event artwork…';
