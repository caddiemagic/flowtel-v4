...
assert.match(server,/process\.env\.ACUITY_USER_ID/);
assert.match(server,/process\.env\.ACUITY_API_KEY/);
for(const browserFile of ['shared/acuity-scheduling.js','shared/womb-magic-booking.js','flow-fm/upcoming-calls/page.js','manager/acuity/app.js']){
  const text=fs.readFileSync(path.join(root,browserFile),'utf8');
  assert.doesNotMatch(text,/process\.env|ACUITY_API_KEY\s*=/,`Secret handling in ${browserFile}`);
}

const suite=fs.readFileSync(path.join(root,'client/index.html'),'utf8');
assert.match(suite,/data-wm-panel/);
assert.doesNotMatch(suite,/href="\/flow-fm\/womb-magic\//);
const nav=fs.readFileSync(path.join(root,'flow-fm/ui.js'),'utf8');
