...
for(const token of ['page.js?v=0.10.80.3','styles.css?v=0.10.80.3']) assert(memberHtml.includes(token),`Member mailbox cache key is missing ${token}.`);
...
for(const token of ['app.js?v=0.10.80.3','Private files awaiting you']) assert(managerHtml.includes(token),`Manager private-media shell is missing ${token}.`);
...
console.log('Flowtel v0.10.80.3 Priestess Mailbox private-media static validation passed.');
