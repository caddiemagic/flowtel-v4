...
for(const token of ['priestess-mailbox-card','priestessMailboxSection','page.js?v=0.10.80.1']) assert(mailboxHtml.includes(token),`Dedicated mailbox HTML is missing ${token}.`);
for(const token of ['listMyPriestessMailbox','sendAudioToConcierge','createMailboxDownloadUrl','markReturnedAudioDownloaded']) assert(mailboxPage.includes(token),`Dedicated mailbox logic is missing ${token}.`);
